...
//                std::cout << "第" << i << "个快照的采样节点个数有" << (*root_nodes).size() << "个\n";
...
//                            std::cout << "选择走3\n";
                            ...
                            int s_search2 = indptr[n];
                            auto e_it = std::upper_bound(ts.begin() + indptr[n],
                                                         ts.begin() + indptr[n + 1], nts);
                            int e_search2 = std::max(int(e_it - ts.begin()) - 1, s_search);

//                            std::cout << "start: " << s_search << " : " << s_search2 << "\n";
//                            std::cout << "end: " << e_search << " : " << e_search2 << "\n";
                        }
                        else
                        {
                            // search for start and end pointer
                            double t_search_s = omp_get_wtime();
//                            std::cout << "当前的history为" << num_history << "\n";
                            if (num_history == 1)
                            {
//                                std::cout << "选择走1\n";
                                // TGAT style
...
//                                    std::cout << indices[i] << " : " << ts[i] << "\n";
//                                std::cout << "将要发现search\n";
//                                std::cout << s_search << " -> " << e_search << "\n";
//                                std::cout << "发现之后开始输出\n";
//                                for (int i = s_search; i <= e_search; i++)
//                                    std::cout << indices[i] << " : " << ts[i] << "\n";


//                                int s_search2 = ts_ptr[num_history - 1 - i][n];
//                                int e_search2 = ts_ptr[num_history - i][n];
//                                std::cout << "start: " << s_search << " : " << s_search2 << "\n";
//                                std::cout << "end: " << e_search << " : " << e_search2 << "\n";
                            }
                            else
                            {
                                // s_search和e_search这里会有问题，怀疑可能往左边多移动了一位
//                                std::cout << "选择走2\n";
                                // DySAT style
                                ...
                                e_search = std::max(int(e_it - ts.begin()) - 1, s_search);
                                while (ts[e_search] >= nts + offset - 1e-7f && e_search >= s_search)
                                    e_search--;
...
//                            std::cout << ts[e_search] << "\n";
//                            std::cout << nts << "\n";
//                            std::cout << nts + offset << "\n";
                            int geshu = neighs;
                            for (EdgeIDType k = e_search; k >= s_search && geshu; k--)
                            {
//                                std::cout << ts[k] << "\n";
...
//            std::cout << "cpp开始采样!!\n";
            ...
            for (int i = 0; i < num_layers; i++)
            {
                ...
                if ((first_layer) || ((prop_time) && num_history == 1) || (recent))
                {
                    ...
                    use_ptr = false;
                use_ptr = false;
//                std::cout << "禁止使用use_ptr!!\n";
//                std::cout << "采样" << root_nodes.size() << "个\n";
                if (i==0)
...
//                std::cout << "采到了" << geshu3 << "个\n";
                geshu3 = 0;
            }
//            std::cout << "最终sample到了" << geshu << "个\n";
        }
};
