...
'''
python -m torch.distributed.launch --nproc_per_node=2 train_dist1.py --data MOOC --config config/TGN.yml --num_gpus 1
python -m torch.distributed.launch --nproc_per_node=2 train_dist.py --data WIKI --config config/TGN.yml --num_gpus 1
            ...
            for _, rows in df[:train_edge_end].groupby(group_indexes[random.randint(0, len(group_indexes) - 1)]):
                ...
                if len(multi_mfgs) == args.num_gpus:
                    ...
                    if mark == 0:
                        ...
                        gathered_train = [None] * (args.num_gpus + 1)
